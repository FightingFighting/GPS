...
from models.mmseg.core.evaluation.metrics import eval_metrics
...
def mDics_and_mIou(loader, model):
    ...
    for batch in pbar:
        ...
        inp = batch['inp']
        pred = torch.sigmoid(model.infer(inp))[0][0]
        label=batch['gt'][0][0]


        label, pred = label.cpu().data.numpy() * 255, pred.cpu().data.numpy() * 255
        pred = (pred > 128).astype(int)
        label = (label > 128).astype(int)
    ...
    ret_metrics = eval_metrics(pred_all, gt_all, 2, ignore_index=0, metrics=['mDice', 'mIoU'])

    result1, result2, result3, result4 = ret_metrics

    print(f"mDice: {result3[1]}")
    print(f"mIoU: {result4[1]}")
