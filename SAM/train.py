...
from utils_.pruning import  prune_by_percentile_gradient_perCell
import wandb
...
def make_data_loaders(tuning_mode = None):
    train_loader = make_data_loader(config.get('train_dataset'), tag='train')
    val_loader = make_data_loader(config.get('val_dataset'), tag='val')
    if tuning_mode == "gps":
        val_loader_grad = make_data_loader(config.get('val_dataset'), tag='val')

        return train_loader, val_loader, val_loader_grad
...
def train(train_loader, model, mask, tuning_mode):
    model.train()
    if local_rank == 0:
    ...
    for batch in train_loader:
        ...
        model.optimize_parameters(mask, tuning_mode)
...
def calculate_gradient(model, val_loader_grad):
    model.optimizer.zero_grad()  # set G's gradients to zero

    for batch in val_loader_grad:
        for k, v in batch.items():
            batch[k] = v.to(device)
        inp = batch['inp']
        gt = batch['gt']
...
def main(config_, save_path, args):
    ...
    if args.tuning_mode == "gps":
        train_loader, val_loader, val_loader_grad = make_data_loaders(args.tuning_mode)
    else:
    ...
    for name, para in model.named_parameters():
        if args.tuning_mode == "gps" or args.tuning_mode == "full":
            pass
        elif args.tuning_mode == "ssf":
            if "image_encoder" in name and "ssf" not in name:
    ...
    print("=========================================None Trainable Parameters=========================================")
    for name, para in model.named_parameters():
    ...
    ###################################################
    if args.tuning_mode == "gps":
        calculate_gradient(model, val_loader_grad)
    ...
    for epoch in range(epoch_start, epoch_max + 1):
        ...
        train_loss_G = train(train_loader, model, mask, args.tuning_mode)
        lr_scheduler.step()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tuning-mode', default=None, type=str, help='Method of fine-tuning (default: None')
    ...
    parser.add_argument("--optimizer", default="", help="optimizer")
    parser.add_argument('--log-wandb', action='store_true', default=False,
                        help='log training and validation metrics to wandb')
    parser.add_argument('--experiment', default='sam', type=str, metavar='NAME',
                        help='name of train experiment, name of sub-folder for output')
    parser.add_argument("--run_name", default="sam_gps", help="run name in experiment")
    ...
    config['model']["args"]['encoder_mode']["tuning_mode"] = args.tuning_mode
