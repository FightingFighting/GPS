...
class SAM(nn.Module):
    def __init__(self, inp_size=None, encoder_mode=None, loss=None):
        ...
        self.image_encoder = ImageEncoderViT(
            ...
            tuning_mode=encoder_mode['tuning_mode']
        )
    ...
    def optimize_parameters(self, mask, tuning_mode):
        self.forward()
        self.optimizer.zero_grad()  # set G's gradients to zero
        self.backward_G()  # calculate graidents for G

        if tuning_mode == "gps":
            self.maks_gradient(mask)

        self.optimizer.step()  # udpate G's weights


    def maks_gradient(self, mask):
        for name, param in self.named_parameters():
            if "iou_prediction_head" not in name:
                mask_ = mask[name]
