...
from .common import LayerNorm2d, MLPBlock, init_ssf_scale_shift, ssf_ada
import math
import warnings
...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        if self.tuning_mode == "gps" or self.tuning_mode ==  "full":
            pass
        elif self.tuning_mode == 'ssf':
                self.ssf_scale_1, self.ssf_shift_1 = init_ssf_scale_shift(self.embed_dim)
        else:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.tuning_mode == "gps" or self.tuning_mode == "full" or self.tuning_mode == "ssf":
            pass
        else:
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.mlp = MLPBlock(embedding_dim=dim, mlp_dim=int(dim * mlp_ratio), act=act_layer, tuning_mode = tuning_mode)
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.tuning_mode == 'ssf':
            qkv = (ssf_ada(self.qkv(x), self.ssf_scale_1, self.ssf_shift_1)).reshape(B, H * W, 3, self.num_heads, -1).permute(2, 0, 3, 1, 4)
        else:
